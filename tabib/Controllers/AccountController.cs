    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Login(LoginModel model, string returnUrl)
        {
            ...
            if (ModelState.IsValid && WebSecurity.Login(model.UserName, model.Password, persistCookie: model.RememberMe))
            {
                ...
                {
                    UserProfile AdminUser = db.UserProfiles.Where(x => x.UserName.Equals(model.UserName)).FirstOrDefault();
                    //count of visit
                    AdminUser.countVisit = AdminUser.countVisit + 1;
                    db.SaveChanges();

                    Session["notification"] = 0;
                    if (model.UserName.ToLower().Equals("admin"))
                        ...
                        Session["notification"] = db.CusConsulation.Include(x => x.UserProfile).Where(x => x.Isreplaied == false
                            && x.UserProfile.UserName.Equals(model.UserName)).Count();
        ...
        public ActionResult LogOff()
        {
            ...
            return RedirectToAction("Index", "Artical");
        }
        ...
        public ActionResult Register()
        {
            ...
            //grph.Flush();
            ////using (FileStream stream = new FileStream("~/Images/img.jpg", FileMode.Create))
            ////{
           ...
           if (!this.IsCaptchaValid("Captcha is not valid"))
            {
                ...
                if (ModelState.IsValid && this.IsCaptchaValid("Captcha is not valid"))
                {
                    ...
                    try
                    {
                        UserProfile user = new UserProfile { DateBirthday = model.DateBirthday, Email = model.email, LastName = model.LastName, UserName = model.UserName };
                        WebSecurity.CreateUserAndAccount(model.UserName, model.Password,
                        propertyValues: new { DateBirthday = model.DateBirthday, Email = model.email, LastName = model.LastName, UserName = model.UserName });
